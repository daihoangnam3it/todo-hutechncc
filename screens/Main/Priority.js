...
import Item from '../../components/itemPrio'
...
const Priority = ({navigation}) => {
  ...
  const getCate=async ()=>{
    const cate=await axios.get(`https://caso-full-test.herokuapp.com/mobile-prio/get/${user}`);
    setCate(cate.data)
  }
  const createCate=async()=>{
    ...
    const res=await axios.post('https://caso-full-test.herokuapp.com/mobile-prio/create',newCate);
    Alert.alert('Ok')
    console.log('moi',res);
  ...
  const handleDelete=async(id)=>{
    ...
    setCate(newCate)
    await axios.get(`https://caso-full-test.herokuapp.com/mobile-prio/delete/${id}`)
    Alert.alert('DDa xoa')
